...
def calculate_hat_position(face_position, hat_size):
    ...
    scale_factor = w * 1 / hat_w
    hat_w = int(hat_w * scale_factor)
    hat_h = int(hat_h * scale_factor)
    pos_hat = (x + w // 2 - hat_w // 2, y - hat_h // 2 + hat_pos)
...
def calculate_mustache_position(nose_position, mustache_size, face_area):
    # should be under the nose
    pass
    (x, y, w, h) = nose_position
    (mustache_w, mustache_h, mustache_o, mustache_of, mustache_y) = mustache_size
    (face_x, face_y, face_w, face_h) = face_area

    scale_factor = face_w / (mustache_o * 9)
    mustache_w = int(mustache_w * scale_factor)
    mustache_h = int(mustache_h * scale_factor)

    print("**************************************")
    print("Nose Position: " + str(nose_position))
    print("Mustache Initial Size: " + str(mustache_size))
    print("Mustache Resized: " + str(mustache_w) + ":" + str(mustache_h))
    print("**************************************")
    pos_mustache = (x + w // 2 - mustache_w // 2, y + h // 2 - mustache_h // 2)
    return pos_mustache, scale_factor, scale_factor
    if mustache_y == 0:
        pos_mustache = (x + w // 2 - mustache_w // 2 - mustache_of, y)
    else:
        pos_mustache = (x + w // 2 - mustache_w // 2 - mustache_of, y + h + mustache_y)
...
pos_uniform = (0, 0)
size_mustache = (1069, 555, 200, 0, 0)
size_mustasche2 = (1000, 601, 268, 15, -15)
pos_mustache = (0, 0)
...
hatImage5 = cv2.imread("marioHatResized.png", cv2.IMREAD_UNCHANGED)
hatImage6 = cv2.imread("LuigiHat.png", cv2.IMREAD_UNCHANGED)
...
mustacheImage = cv2.imread("mustacheandnose.png", cv2.IMREAD_UNCHANGED)
...
while True:
    ...
    for x, y, w, h in faces:
        ...
        if hat_status == 5:
            pos_hat, scale_x, scale_y = calculate_hat_position((x, y, w, h), size_hat5)
            frame = transparent_overlay(frame, hatImage5, pos_hat, scale_x, scale_y)
